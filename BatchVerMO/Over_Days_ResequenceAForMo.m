function As=Over_Days_ResequenceAForMo(As,correlation_thresh,max2max2nd,skewnessthresh)
% General description: This function is based on Over_Days_ResequenceA. For
% BatchVerMo, it applies the first day's As resequenced result to all the
% other days. Below is the documentation for "Over_Days_ResequenceA".
% This function puts neurons that are roughly checked(findn1n2)
    ...
    for i=1:length(As)-1
        ...
        if or(isempty(ns_storage),size(ns_storage,1)==1) % no need to re-sequence them.
            ...
            for j=1:length(As_all)
                As_all{j}{i+1}=As_all{j}{i+1}(:,all_location);
            end
        end
    end 
end
