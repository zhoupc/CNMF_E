function [c, s, options,ind_success] = deconvolveCa_NoNoise(y, varargin)
...
if isempty(options.pars)
    while and(k<2,~ind_success)
        switch options.type
            case 'ar1'
                options.pars = estimate_time_constant_NoNoise(y, 1);
                if length(options.pars)~=1
                    display('ar1 is not suitable, trying ar2.')
                    options.type='ar2';
                    k=k+1;
                else
                    ind_success=true;
            ...
            case 'ar2'
                options.pars = estimate_time_constant_NoNoise(y, 2);
                if length(options.pars)~=2
                    display('ar2 is not suitable, trying ar1.')
                    options.type='ar1';
                    k=k+1;
                else
                    ind_success=true;
            ...
            case 'exp2'
                g = estimate_time_constant_NoNoise(y, 2);
                if length(options.pars)~=2
                    display('exp2 is not suitable, trying ar1.')
                    options.type='ar1';
                    k=k+1;
                else
            ...
            case 'kernel'
                g = estimate_time_constant_NoNoise(y, 2);
                if length(options.pars)~=2
                    display('exp2 is not suitable, trying other methods.')
                    options.type='ar1';
                    k=k+1;
                else
                    taus = ar2exp(g);
...
if ind_success==false
    c = ci_raw;
    s = [];
    options.pars = [];
else % run deconvolution
    c = y;
    s = y;
