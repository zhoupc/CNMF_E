...
if (~exist('patch_sz', 'var'))||(isempty(patch_sz))||(max(patch_sz(:))==1)
    ...
    if nk>1  % detrend data
        ...
        [results, center, Cn, PNR] = greedyROI_endoscope2(Ydt, K, options, debug_on, save_avi);
    else    % without detrending
        [results, center, Cn, PNR] = greedyROI_endoscope2(Y, K, options, debug_on, save_avi);
    end
    obj.A = results.Ain;
    obj.C = results.Cin;
...
for mr = 1:nr_patch
    ...
    for mc = 1:nc_patch
        ...
        if nk>1
            ...
            [tmp_results, tmp_center, tmp_Cn, tmp_PNR, save_avi] = greedyROI_endoscope2(Ypatch_dt, K, tmp_options, debug_on, tmp_save_avi);
        else
            [tmp_results, tmp_center, tmp_Cn, tmp_PNR, save_avi] = greedyROI_endoscope2(Ypatch, K, tmp_options, debug_on, tmp_save_avi);
        end
        close(gcf);
