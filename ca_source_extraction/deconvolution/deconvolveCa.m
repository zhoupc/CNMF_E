...
if isempty(y)   % get the default values 
    c = []; s=[]; return; 
end
...
if ~isempty(varargin) && isstruct(varargin{1}) 
    ...
    options.type = 'ar1';   % model type 
    options.pars = [];      % model parameters 
    options.sn = [];        % noise level 
    options.b = 0;          % baseline 
    options.lambda = 0;     % penalty parameter 
    options.optimize_b = false;  % boolean, optimize the baseline 
    options.optimize_pars = false;  % boolean, optimize model parameters 
    options.optimize_smin = false;  % boolean, optimize smin 
    options.method = 'constrained'; % default method to use: constrained foopsi 
    options.window = 200;           % window of the kernel width 
    options.shift = 100;            % shift in window for online NNLS
    options.smin = 0;               % threshold of the spike size 
    options.maxIter = 10;           % maximum iterations for running deconvolution 
    options.thresh_factor = 1.0;    % the maximum RSS relative to sigma^2*T 
    options.extra_params = [];      % extra parameters for different deconvolution methods
end
