...
addpath(genpath('\\feevault\shared\EmilyShijieShared\CNMF_E-master'))

%% select data and map it to the RAM
nam = '\\feevault\shared\EmilyShijieShared\TestDatasets\CaELM_row3289.mat';
cnmfe_choose_data;
...
tic;
cnmfe_update_BG;
fprintf('Time cost in updating the background:        %.2f seconds\n', toc);
%update spatial & temporal components
tic;
for m=1:2
    %temporal
...
neuron.Coor = plot_contours(neuron.A, Cnn, 0.8, 1, [], neuron.Coor, 2);
colormap winter;
axis equal; axis off;
...
plot_contours(neuron.A, Cn, 0.8, 1, [], neuron.Coor, 2);
colormap winter;
title('contours of estimated neurons');
