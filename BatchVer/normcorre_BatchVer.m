function [M_final,shifts_g,template] = normcorre_BatchVer(Y,options,template,sizes,As)
                ...
                for j = 1:length(yy_s)           
                    for k = 1:length(zz_s)
                        ...
                        if all([length(xx_s),length(yy_s),length(zz_s)] == 1)
                            display('line 307')
                            for ni=1:sizes(ii)
                                M_fin{ni}{i,j,k} = shift_reconstruct(Yt,shifts_temp(i,j,k,:),diff_temp(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value);
                            end
                        end                                               
                    end
                end
            ...
            switch lower(options.shifts_method)
                case 'fft'
                            ...
                            for i = 1:length(xx_uf)
                                for j = 1:length(yy_uf)
                                    for k = 1:length(zz_uf)
                                        Y_one_neuron=reshape(As{ii}(:,ni),d1,d2);
                                        extended_grid = [max(xx_us(i)-overlap_post(1),1),min(xx_uf(i)+overlap_post(1),d1),max(yy_us(j)-overlap_post(2),1),min(yy_uf(j)+overlap_post(2),d2),max(zz_us(k)-overlap_post(3),1),min(zz_uf(k)+overlap_post(3),d3)];
                                        %I_temp = Yt(extended_grid(1):extended_grid(2),extended_grid(3):extended_grid(4),extended_grid(5):extended_grid(6));
                                        I_temp = Y_one_neuron(extended_grid(1):extended_grid(2),extended_grid(3):extended_grid(4),extended_grid(5):extended_grid(6));
                    ...
                    for ni=1:sizes(ii)
                        M_fin_tmp=M_fin{ni};
                        if flag_interp
                            Mf_each{ni} = cell2mat_ov_sum(M_fin_tmp,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY,Bs) - add_value;
                        else
                            Mf_each{ni} = cell2mat_ov(M_fin_tmp,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                        end
                        Mf_each{ni}(Mf_each{ni}<minY)=minY;
                        Mf_each{ni}(Mf_each{ni}>maxY)=maxY;
                        Mf_temp=[Mf_temp reshape(Mf_each{ni},[],1)];
                    end
                    Mf{ii}=Mf_temp;
                otherwise
                    shifts(ii).shifts_up = shifts(ii).shifts;
                    if nd == 3
                        ...
                        for ni=1:sizes(ii)
                            Y_one_neuron=reshape(As{ii}(:,ni),d1,d2);
                            M_fin{ni} = imwarp(Y_one_neuron,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method);
                            Mf_temp=[Mf_temp reshape(M_fin{ni},[],1)];
                        end
                        Mf{ii}=Mf_temp;
                    end
            end
