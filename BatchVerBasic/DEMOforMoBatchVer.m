...
scriptname='moBatchVer6922.sbatch';
shellname='moBatchVer6922.sh';
...
filename=['LogisticsLIST=${LogisticsLIST}\n',...
    'LogisticsNAME=${LogisticsLIST[$SLURM_ARRAY_TASK_ID]}'];
fprintf(fileID,'%s\n',filename);

MATLABcommands=['source ''%s''',...
    'module add mit/matlab/2016b\n',...
    'matlab -nodisplay -singleCompThread -r "display(${LogisticsLIST}); display(${LogisticsNAME}); addpath(genpath(''%s''));\\\n',...
    'load(fullfile(''%s'',${LogisticsNAME})); cnmfeMoBatchVer_ClusterPart"'];
fprintf(fileID,MATLABcommands,fullfile(jobdir,shellname),codeDir,outputdir);
