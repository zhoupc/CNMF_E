...
tau_rise = 0.1; 
nframe_decay = ceil(neuron_raw.Fs*10*tau_decay); 
bound_pars = false; 
...
num2read= numFrame-sframe+1;             % user input: how many frames to read   (optional, default: until the end)
...
%% iteratively update the results
maxIter = 4; 
for miter=1:maxIter
    ...
    if miter<2
        merge_thr = [0.1, 0.7, 0];     % thresholds for merging neuron (spatial overlap, temporal correlction, spike correlation)
    else
        merge_thr = [0.3, 0.6, 0];
    end
...
% Ybg = neuron.localBG(Y-neuron.A*neuron.C, ssub, rr, active_px, sn, 5); % estiamte local background.
...
figure;
plot_contours(neuron.A, Cn, 0.7, 0, [], neuron.Coor, 2);
colormap winter;
...
% neuron.Cn = Cn;
% neuron.runMovie(Ysignal, [0, max(Ysignal(1:10:end))*0.8], save_avi, avi_name);
